...
Use markdown formatting for structure and readability.`;
  ...
  const testApiConnection = async () => {
    setApiTestStatus({ status: 'loading', message: 'Testing API connection...' });
    try {
      const response = await generateText('Say "Connection successful" if you receive this message.');
      setApiTestStatus({ 
        status: response.includes('Connection successful') ? 'success' : 'error',
        message: response
      });
    } catch (error) {
      setApiTestStatus({ 
  ...
  return (
    <main className="container mx-auto p-4 sm:p-8">
      ...
      {/* API Test Section */}
      <section id="api-test" className="my-8">
        <div className="max-w-5xl mx-auto bg-gray-50 p-4 rounded-lg border border-gray-200">
          <h3 className="text-xl font-semibold mb-2">API Connection Test</h3>
          <p className="text-sm text-gray-600 mb-4">Click the button below to test the OpenRouter API connection.</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <span className="text-sm text-gray-500">This tests if the OpenRouter API is properly connected.</span>
          </div>
          
          <div className="mt-4 pt-4 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              Having connection issues? Try our <a href="./network-test.html" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Network Diagnostic Tool</a> for detailed troubleshooting.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="api-connection-test" className="mt-16">
        <div className="bg-white p-8 rounded-lg shadow-xl text-center">
          <h2 className="text-3xl font-bold mb-4">API Connection Test</h2>
          <p className="text-gray-600 mb-6">Verify your connection to the OpenRouter API. This is important for the Dynamic Playbook and other AI features to function correctly.</p>
          <button 
            id="test-api-btn" 
            className="bg-accent-1 text-white font-bold py-3 px-6 rounded-lg hover:opacity-90 transition-opacity shadow-lg disabled:opacity-50"
            onClick={testApiConnection}
            disabled={apiTestStatus.status === 'loading'}
          >
            {apiTestStatus.status === 'loading' ? 'Testing...' : 'Test API Connection'}
          </button>
          {apiTestStatus.status !== 'idle' && (
            <div className={`mt-4 p-4 rounded-lg text-sm ${apiTestStatus.status === 'success' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
